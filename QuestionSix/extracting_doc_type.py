...
def fetch_total_documents_for_year_with_keywords(year, keywords):
    ...
    formatted_list_keywords = f"({keywords})"
    params = {
...
if response.status_code == 200:
    ...
    for document in json_response.get('result', []):
        contributors = document.get('document_type')
        all_descriptions.append(contributors.get('description',[]))
        all_codes.append(contributors.get('code',[]))

    print(f"All Description: {all_descriptions}")
